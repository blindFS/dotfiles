...
local function show_ippopup_infos(n_graph)
  ...
  if data[n_graph][interface.."_state"] == "up" then
    if data[n_graph][interface.."_carrier"] == "1" then --get local ip configuration
      ...
      local ext_ip = awful.util.pread("curl --silent --connect-timeout 1 -S http://ipecho.net/plain; 2>&1")
...
function new(args,newimage)
    ...
    n_graph.widget.image = capi.image(newimage)
...
--    n_graph.add_value = add_value
    ...
    n_graph.layout = args.layout or layout.horizontal.rightleft
    update_net(n_graph)
--    set_ippopup(n_graph)
    return n_graph
end
